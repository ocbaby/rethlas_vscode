...
2. If the current host supports sub-agents, spawn one sub-agent per decomposition plan. Otherwise, work through the plans sequentially while keeping separate notes for each plan.
...
7. Wait for all sub-agents to finish, then gather their reports. In sequential mode, gather the per-plan notes before continuing.
...
- any sub-agent tools exposed by the current host
