...
Use the standalone verification workflow for proof verification instead of a local HTTP service or model-only checking.
Only run verification when a full proof of the whole problem has been assembled in `blueprint.md`. Do not run it on partial proofs, incomplete branches, isolated lemmas, or drafts that have made no real progress on the full theorem.
In GitHub Copilot, prefer the `Verify Generated Blueprint` handoff or `/verify-rethlas`.
In Claude Code, switch to `agents/verification` and verify the same statement and `blueprint.md` against `agents/verification/AGENTS.md`.
Only publish `results/{problem_id}/blueprint_verified.md` after a verification pass reports zero critical errors and zero gaps.
If verification is unavailable in the current environment, stop at `blueprint.md` and state that verification is still pending.

## Output Contract

Use standard Markdown math delimiters `$...$` for inline math and `$$...$$` for displayed equations.
Do not wrap mathematical expressions in backticks.
